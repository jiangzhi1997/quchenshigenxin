...
class Cart extends Component{
    ...
    delete=(item)=>{
        // for(let i=0;shopArr.length;i++){

        // }
        var index=item.target.getAttribute(item);

        var shopArr=this.state.shopArr;
        console.log(index)
        shopArr.splice(index,1)
        this.setState({shopArr:shopArr})
    }
        ...
        return(
            <div className="Cart">
                <div className="cart_title">
                    <span onClick={this.Cartout}><Icon type="left" /></span>
                    <span>购物车</span>
                    <span>编辑</span>
                </div>
                <div className="shop-CART">
                    {
                      ...
                      {
                          this.state.shopArr.map((item,index)=><li key={index}>
                                <div className="cart-Img">
                                    <img src={item.img_src} alt='' />
                                </div>
                                <div>
                                    <span>{item.name}</span>
                                    <span style={{'fontSize':'16px','color':'#FF82A6'}}>￥{item.app_price/100}</span>
                                    <p>{item.market_price}x{item.num}</p>
                            ...
                            <div>
                                <Button type="primary" className="cart-btn" onClick={this.goHome}>去首页逛逛</Button>
                            </div>
                    </div>
                    }
                </div>
